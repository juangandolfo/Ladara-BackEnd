...
export class Migrations1787503756066 implements MigrationInterface {
    ...
    private async constraintExists(queryRunner: QueryRunner, tableName: string, constraintName: string): Promise<boolean> {
        const rows = await queryRunner.query(
            `SELECT COUNT(*) as count FROM information_schema.key_column_usage WHERE table_schema = DATABASE() AND table_name = ? AND constraint_name = ?`,
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        if (!(await this.tableExists(queryRunner, 'product'))) {
        ...
        if (!(await this.tableExists(queryRunner, 'order'))) {
        ...
        if (!(await this.tableExists(queryRunner, 'order_item'))) {
        ...
        if (!(await this.tableExists(queryRunner, 'discount'))) {
        ...
        if (!(await this.constraintExists(queryRunner, 'order_item', 'FK_646bf9ece6f45dbe41c203e06e0'))) {
            await queryRunner.query(`ALTER TABLE \`order_item\` ADD CONSTRAINT \`FK_646bf9ece6f45dbe41c203e06e0\` FOREIGN KEY (\`orderId\`) REFERENCES \`order\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        }

        if (!(await this.constraintExists(queryRunner, 'order_item', 'FK_904370c093ceea4369659a3c810'))) {
            await queryRunner.query(`ALTER TABLE \`order_item\` ADD CONSTRAINT \`FK_904370c093ceea4369659a3c810\` FOREIGN KEY (\`productId\`) REFERENCES \`product\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        }

        if (!(await this.constraintExists(queryRunner, 'order', 'FK_caabe91507b3379c7ba73637b84'))) {
        ...
        if (!(await this.constraintExists(queryRunner, 'discount', 'FK_cf500130a0d2ac5af6109068e63'))) {
